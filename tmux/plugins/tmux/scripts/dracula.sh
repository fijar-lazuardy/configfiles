...
main()
{
 ...
 # done
  ...
  for plugin in "${plugins[@]}"; do
    ...
    elif [ $plugin = "music-artist" ]; then
      IFS=' ' read -r -a colors <<<$(get_tmux_option "@dracula-spotify-tui-colors" "pink dark_gray")
      script="#($current_dir/artist.sh)"

    elif [ $plugin = "music-song" ]; then
      IFS=' ' read -r -a colors <<<$(get_tmux_option "@dracula-spotify-tui-colors" "light_purple dark_gray")
      script="#($current_dir/track.sh)"

    elif [ $plugin = "music-album" ]; then
      IFS=' ' read -r -a colors <<<$(get_tmux_option "@dracula-spotify-tui-colors" "cyan dark_gray")
      script="#($current_dir/album.sh)"

    elif [ $plugin = "music-logo" ]; then
      IFS=' ' read -r -a colors <<<$(get_tmux_option "@dracula-spotify-tui-colors" "pink dark_gray")
      script="#($current_dir/logo.sh)"
